...
class ReservationCreateService
{
  ...
  public function execute(CreateGuestDTO $dto): Guest
  {
    return DB::transaction(function () use ($dto) {
      ...
      $room = Room::with('tariffs')->findOrFail($dto->reservation->room_id);

      $total = $this->calculatorService->calculate(
        $room,
        $dto->reservation->checkin_at,
        $dto->reservation->checkout_at,
        $dto->reservation->adults,
        $dto->reservation->children,
        $dto->reservation->regime_id
      );

      $reservation->update(['total' => $total]);
      $this->reservationPaymentService->createInitialPayment($reservation, $total);
